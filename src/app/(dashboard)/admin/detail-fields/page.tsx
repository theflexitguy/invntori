  ...
  return (
    <div className="p-6 xl:p-8 w-full max-w-2xl">
      ...
      <div className="flex items-start justify-between mb-6 mt-4">
        <div>
          <h2 className="text-2xl font-bold text-white">Detail Fields</h2>
              ...
              <div key={field.id} className="flex items-center justify-between px-5 py-4">
                ...
                {user?.isAdmin && (
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(field)} className="px-2.5 py-1.5 text-xs rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-colors">Edit</button>
                    <button onClick={() => setConfirmDelete(field)} className="px-2.5 py-1.5 text-xs rounded-lg text-red-400 hover:bg-red-400/10 transition-colors">Delete</button>
                  </div>
                )}
              </div>
            ))}
