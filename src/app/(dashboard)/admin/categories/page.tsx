              ...
              <div key={cat.id} className="flex items-center justify-between px-5 py-4">
                ...
                {user?.isAdmin && (
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(cat)} className="px-2.5 py-1.5 text-xs rounded-lg text-gray-400 hover:text-white hover:bg-white/5 transition-colors">Edit</button>
                    <button onClick={() => setConfirmDelete(cat)} className="px-2.5 py-1.5 text-xs rounded-lg text-red-400 hover:bg-red-400/10 transition-colors">Delete</button>
                  </div>
                )}
              </div>
            ))}
