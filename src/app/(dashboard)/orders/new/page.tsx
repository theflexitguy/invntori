...
function blankItem(): LineItem {
  return { key: nextKey++, productID: "", productName: "", quantity: "1", unit: "", unitCost: "" };
}
...
export default function LogPurchaseOrderPage() {
  ...
  const validItems = useMemo(
    () => items.filter((i) => i.productID && i.productName.trim() && parseFloat(i.quantity) > 0),
    [items]
  );
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Header document — includes items array for fast web display
      batch.set(poRef, {
